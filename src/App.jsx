...
import {
  ...
  PauseCircle, PlayCircle, ListChecks, MessageSquareText,
} from 'lucide-react';
...
function getChargeDerived(charge, today) {
  ...
  const followUps = charge.followUps || [];
  const latestFollowUp = followUps.reduce((latest, f) => (!latest || f.date > latest.date ? f : latest), null);
  let needsFollowUp = false;
  if (!isPaid && daysOverdue > 0) {
    if (!latestFollowUp) needsFollowUp = true;
    else {
      const lf = new Date(latestFollowUp.date + 'T00:00:00');
      if (daysBetween(lf, today) >= 3) needsFollowUp = true;
    }
  }
  return { due, isPaid, isPartial, daysOverdue, status, needsFollowUp, latestFollowUp, followUpCount: followUps.length };
}
...
export default function AgencyCollections() {
  ...
  const [followUpModal, setFollowUpModal] = useState(null); // charge id or null
  const [followUpForm, setFollowUpForm] = useState({ date: todayStr(), note: '' });
  ...
  function submitModal(e) {
    ...
    if (modal.type === 'client') {
      ...
      const charge = {
        ...
        followUps: prior ? (prior.followUps || []) : [],
        notes: (form.notes || '').trim(),
      };
  ...
  function openFollowUp(charge) {
    setFollowUpModal(charge.id);
    setFollowUpForm({ date: todayStr(), note: '' });
  }
  function submitFollowUp(e) {
    e.preventDefault();
    if (!followUpForm.date) return;
    const entry = { id: newId(), date: followUpForm.date, note: (followUpForm.note || '').trim() };
    persist({ ...data, charges: data.charges.map((c) => (c.id === followUpModal ? { ...c, followUps: [...(c.followUps || []), entry] } : c)) });
    setFollowUpForm({ date: todayStr(), note: '' });
  }
  function performDelete() {
    ...
    if (type === 'client') {
      ...
      persist({ ...data, charges: data.charges.map((c) => (c.id === confirmDelete.chargeId ? { ...c, followUps: (c.followUps || []).filter((f) => f.id !== confirmDelete.entryId) } : c)) });
    }
  ...
  return (
    <div style={{ background: palette.bg, fontFamily: sansFont, color: palette.textPrimary, minHeight: '100vh' }} className="w-full">
      ...
      {followUpModal && (() => {
        const charge = enrichedCharges.find((c) => c.id === followUpModal);
...
function FollowUpsView({ data, totals, buckets, bucketDefs, missingInvoices, filtered, search, setSearch, statusFilter, setStatusFilter, kindFilter, setKindFilter, clientFilter, setClientFilter, sortBy, setSortBy, onAddCharge, onEditCharge, onOpenFollowUp, confirmDelete, setConfirmDelete, performDelete, goToClients }) {
    ...
    <>
      ...
      <div className="hidden md:block rounded-xl overflow-hidden" style={{ border: `1px solid ${palette.border}` }}>
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {filtered.map((r) => (
              <ChargeRow key={r.id} r={r} onEdit={() => onEditCharge(r)} onOpenFollowUp={() => onOpenFollowUp(r)} confirmDelete={confirmDelete} setConfirmDelete={setConfirmDelete} onDelete={performDelete} />
            ))}
          </tbody>
        </table>
      ...
      <div className="md:hidden flex flex-col gap-3">
        {filtered.map((r) => (
          <ChargeMobileCard key={r.id} r={r} onEdit={() => onEditCharge(r)} onOpenFollowUp={() => onOpenFollowUp(r)} confirmDelete={confirmDelete} setConfirmDelete={setConfirmDelete} onDelete={performDelete} />
        ))}
...
function ChargeRow({ r, onEdit, onOpenFollowUp, confirmDelete, setConfirmDelete, onDelete }) {
  ...
  return (
    <tr style={{ borderTop: `1px solid ${palette.border}` }}>
      ...
      <td className="px-4 py-3 align-top">
        <button onClick={onOpenFollowUp} className="text-left" style={{ color: palette.textSecondary }}>
          <div className="text-xs">{r.latestFollowUp ? formatDate(r.latestFollowUp.date) : 'Never'}</div>
          {r.latestFollowUp && r.latestFollowUp.note && <div className="text-xs mt-0.5 italic" style={{ color: palette.textMuted, maxWidth: 180 }}>&ldquo;{truncate(r.latestFollowUp.note, 42)}&rdquo;</div>}
        </button>
        {r.needsFollowUp && <div className="text-xs mt-0.5 font-medium" style={{ color: palette.gold }}>Flag: follow up</div>}
      </td>
      <td className="px-4 py-3 align-top">
        <div className="flex items-center justify-end gap-1">
          {!r.isPaid && <button onClick={onOpenFollowUp} title="Log a follow-up" className="p-1.5 rounded-md" style={{ color: palette.textMuted }}><CheckCircle2 size={15} /></button>}
...
function ChargeMobileCard({ r, onEdit, onOpenFollowUp, confirmDelete, setConfirmDelete, onDelete }) {
  const confirming = confirmDelete && confirmDelete.type === 'charge' && confirmDelete.id === r.id;
  return (
    <div className="rounded-xl p-4" style={{ background: palette.surface, border: `1px solid ${palette.border}` }}>
      ...
      <button onClick={onOpenFollowUp} className="w-full text-left mb-2" style={{ color: palette.textSecondary }}>
        <div className="text-xs">
          Last follow-up: {r.latestFollowUp ? formatDate(r.latestFollowUp.date) : 'Never'}
          {r.needsFollowUp && <span className="ml-2 font-medium" style={{ color: palette.gold }}>Flag</span>}
        </div>
        {r.latestFollowUp && r.latestFollowUp.note && <div className="text-xs mt-0.5 italic" style={{ color: palette.textMuted }}>&ldquo;{truncate(r.latestFollowUp.note, 60)}&rdquo;</div>}
      </button>
      <div className="flex items-center justify-end gap-1">
        {!r.isPaid && <button onClick={onOpenFollowUp} className="p-1.5" style={{ color: palette.textMuted }}><CheckCircle2 size={15} /></button>}
...
function FollowUpModal({ charge, form, setForm, onSubmit, onClose, confirmDelete, setConfirmDelete, onDelete }) {
  const history = [...(charge.followUps || [])].sort((a, b) => (a.date < b.date ? 1 : -1));
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl p-6 max-h-[85vh] overflow-y-auto scrollbar-thin" style={{ background: palette.surface, border: `1px solid ${palette.borderLight}`, fontFamily: sansFont, color: palette.textPrimary }}>
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-lg font-semibold flex items-center gap-2"><MessageSquareText size={18} style={{ color: palette.gold }} /> Follow-up log</h2>
          <button type="button" onClick={onClose} style={{ color: palette.textMuted }}><X size={18} /></button>
        </div>
        <div className="text-xs mb-4" style={{ color: palette.textSecondary }}>
          {charge.clientName} &middot; {charge.engagementLabel} &middot; {charge.label}
        </div>

        <form onSubmit={onSubmit} className="mb-5">
          <div className="grid grid-cols-2 gap-3 mb-3">
            <Field label="Date">
              <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} style={inputStyle} className="w-full px-3 py-2 rounded-lg text-sm" />
            </Field>
          </div>
          <Field label="Comment (optional)" full>
            <textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} style={inputStyle} rows={2} className="w-full px-3 py-2 rounded-lg text-sm resize-none" placeholder="e.g. Called, promised payment by Friday" />
          </Field>
          <div className="flex justify-end mt-3">
            <button type="submit" className="px-4 py-2 rounded-lg text-sm font-medium" style={{ background: palette.gold, color: '#151107' }}>Log follow-up</button>
          </div>
        </form>

        {history.length > 0 && (
          <div>
            <div className="text-xs font-semibold mb-2" style={{ color: palette.textMuted, letterSpacing: '0.04em' }}>HISTORY ({history.length})</div>
            <div className="flex flex-col gap-2">
              {history.map((f) => {
                const confirming = confirmDelete && confirmDelete.type === 'followup' && confirmDelete.chargeId === charge.id && confirmDelete.entryId === f.id;
                return (
                  <div key={f.id} className="flex items-start justify-between gap-2 p-2.5 rounded-lg" style={{ background: palette.bg, border: `1px solid ${palette.border}` }}>
                    <div className="min-w-0">
                      <div className="text-xs font-medium" style={{ fontFamily: monoFont, color: palette.textSecondary }}>{formatDate(f.date)}</div>
                      {f.note && <div className="text-sm mt-0.5" style={{ color: palette.textPrimary }}>{f.note}</div>}
                    </div>
                    {confirming ? (
                      ...
                      <button onClick={() => setConfirmDelete({ type: 'followup', chargeId: charge.id, entryId: f.id })} className="p-1 rounded shrink-0" style={{ color: palette.textMuted }}><Trash2 size={13} /></button>
                    )}
                  </div>
                );
              })}
